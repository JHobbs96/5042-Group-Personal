...
{
    const char* rpcs[] = { "connect", "disconnect", "status", "quiz", "answer" };
    char buffer[1024] = { 0 };
...
//            const char* welcome = ("Welcome to the Quiz Game where you will be asked"
//                   "questions regarding Computer Science and"
//                   "Software Engineering");
//            send(this->m_socket, welcome, strlen(welcome) + 1, 0)
//
//            readFile(questionArray);
//
//            const char* q = questionArray[0];
//            send(this->m_socket, q, strlen(q) + 1, 0);)
        ...
        //Get Question
        else if((bConnected == true) && (aString == "question")){
//            bStatusOk = ProcessQuestionRPC();
        }

        //Answer Question
        else if((bConnected == true) && (aString == "answer")){
...
//    send(this->m_socket, szBuffer, strlen(szBuffer) + 1, 0);
    ...
    //Need to find a way to convert questions from string to char*
//    const char* q = questionArray[0];
//    send(this->m_socket, q, strlen(q) + 1, 0);
...
//bool RPCImpl::ProcessQuestionRPC(std::vector<std::string>& arrayTokens)
//{
//    char szBuffer[16];
//
//
